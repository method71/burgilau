const priceElement = document.querySelector("[data-price]");
  ...
  if (heroTrack && heroSlides.length) {
    ...
    [firstClone, lastClone].forEach(clone => {
      ...
      clone.querySelectorAll("a,button,input").forEach(element => element.tabIndex = -1);
    });
    ...
    function updateHeroFade() {
      ...
      renderedHeroSlides.forEach(slide => {
        ...
        slide.style.opacity = String(1 - distance * .65);
      });
    ...
    function finishHeroDrag(event, cancelled = false) {
      ...
      const swipeThreshold = Math.min(heroTrack.clientWidth * .14, 72);
    ...
    }, { root: heroTrack, threshold: [.55, .75, .95] });
