...
test("updates the estimate and keeps a single hero slide static", async ({ page }) => {
  const browserErrors = [];
  ...
  await expect(page.locator("[data-hero-slide]:not([data-loop-clone])")).toHaveCount(1);
  await expect(page.locator("[data-hero-controls]")).toBeHidden();
  await expect(realHeroCtas).toHaveCount(0);
  await expect(hero).not.toHaveAttribute("tabindex", /.+/);
